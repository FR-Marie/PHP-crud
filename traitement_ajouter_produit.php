...
//vérif de l'existence de la variable $_FILES["image_produit"]
...
//INSERER LE NOUVEAU PRODUIT
    if($dbh){
        $sql = "INSERT INTO `produits_peluches`(`id_produit`, `nom_produit`, `marque_produit`, `sousTitre_produit`, `description_produit`, `prix_produit`, `stock_produit`, `date_depot`, `image_produit`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
 

        $nom_du_produit = trim(htmlspecialchars($_POST["nom_produit"]));
        $marque_du_produit = trim(htmlspecialchars($_POST["marque_produit"]));
        $sousTitre_du_produit = trim(htmlspecialchars($_POST["sousTitre_produit"]));
        ...
        $requeteAjout = $dbh->prepare($sql);

        //Je lie les paramètres entrés dans le formulaire à ma BD
        $requeteAjout->bindParam(1, $_POST["id_produit"]);
        $requeteAjout->bindParam(2, $_POST["nom_produit"]);
        $requeteAjout->bindParam(3, $_POST["marque_produit"]);
        $requeteAjout->bindParam(4, $_POST["sousTitre_produit"]);
        ...
        $requeteAjout->execute([
            ...
            trim(htmlspecialchars($_POST["marque_produit"])),
            trim(htmlspecialchars($_POST["sousTitre_produit"])),
        ...
        if(isset($nom_du_produit) && !empty($nom_du_produit) && isset($marque_du_produit) && !empty($marque_du_produit) && isset($sousTitre_du_produit) && !empty($sousTitre_du_produit) && isset($description_du_produit) && !empty($description_du_produit) &&
        isset($prix_du_produit) && !empty($prix_du_produit) && isset($stock_du_produit) && !empty($stock_du_produit) && isset($date_du_produit) && !empty($date_du_produit) && isset($image_du_produit) && !empty($image_du_produit)){


            if($requeteAjout){
                ?>
                <div class="alert alert-success w-50 m-auto text-center">
                    <p>Ajout de produit réussit</p>
                    <a href="accueil.php" class="btn btn-warning">Retour accueil</a>
                </div>
            <?php
            }else{
                ?>
                <div class="alert alert-danger w-50 m-auto">
                    <p>Echec de l'ajout du produit</p>
                    <a href="accueil.php" class="btn btn-warning">Retour accueil</a>
                </div>
            <?php
            }
